...
<div class="container" style="min-height: 100%;">
  <h3>PUSAT KARIR</h3> <br>
  <div class="row">
    <div class="col-sm" style="margin: 1%">
      <?php
      while($listjob = mysqli_fetch_array($getjob)){
        echo "<h5>".$listjob['namaJob']."</h5>";
        echo "<p>".$listjob['deskripsiJob']."</p>";
        }?>
    </div>
    ...
    <div class="col-sm">
      <form>
        <div class="formpendaftaran">
          ...
          <label>Posisi yang dipilih</label>
          <select class="custom-select" required name="posisi">
            <option value="" selected disabled hidden></option>

            <?php
            $query = mysqli_query($connect, "SELECT * FROM list_job");

            while ($data = mysqli_fetch_array($query))

            {

            echo "<option value='". $data['namaJob']."'>".$data['namaJob']."</option>";

            }
            ?>
          </select>
          <label>Email</label>
          <input type="email" name="emailpengirim" class="form-control" required>
          <label>No. Handphone / telp</label>
          <input type="text" name="nohp" class="form-control" required>
          <br><label><b>Lampiran</b></label>
          <div class="input-group mb-3">
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" required>
              <label class="custom-file-label" for="inputGroupFile01">CV (.pdf)</label>
            </div>
          </div>
          <div class="input-group mb-3">
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" required>
              <label class="custom-file-label" for="inputGroupFile01">Pas Foto 3x4 (.jpg)</label>
            </div>
          </div>
          <div class="input-group mb-3">
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" required>
              <label class="custom-file-label" for="inputGroupFile01">KTP (.pdf)</label>
            </div>
          </div>
          <button type="submit" class="btn btn-success" style="width: 100%">Kirim Lamaran</button>
        </div>
      </form>
    </div>
